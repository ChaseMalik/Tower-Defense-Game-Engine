...
public class LevelDisplayCell extends HBox {
	...
	/**
	 * Creates the HBox that displays the enemy image with a numeric text field
	 * for enemy number selection
	 * 
	 * @return
	 */
	...
	private HBox createEnemiesBox() {
		...
		for (BaseEnemy enemy : myEnemies) {
			...
			ImageView enemyImg = StringToImageViewConverter.getImageView(
					FIT_SIZE, FIT_SIZE, enemy.getImagePath());
			enemyImg.setFitHeight(FIT_SIZE);
			enemyImg.setFitWidth(FIT_SIZE);
			NumericalTextField enemiesCountField = new NumericalTextField(
					TEXT_FIELD_WIDTH);
	...
	/**
	 * Creates the slot that display level number, a level time input option
	 * 
	 * @return
	 */
	...
	private VBox createLevelInfoBox() {
		...
		secondsLabel.textProperty().bind(
				MultiLanguageUtility.getInstance().getStringProperty(
						Constants.SECONDS));
		...
		levelInfoBox.getChildren().addAll(levelLabel, timeAndSeconds);
		return levelInfoBox;
	}
	...
	public BaseLevel generateLevel() {
		...
		for (BaseEnemy enemy : myEnemies) {
			level.addEnemyCountPair(new EnemyCountPair(enemyToEnemyCountMap
					.get(enemy).getNumber(), enemy));
		}
